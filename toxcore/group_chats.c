...
{
    return (GC_MAX_PACKET_SIZE - length) % GC_MAX_PACKET_PADDING;
}
...
{
    uint8_t plain[GC_MAX_PACKET_SIZE];
                                  ...
                                  uint32_t length)
{
    if (length != sizeof(uint32_t) + MAX_GC_PASSWORD_SIZE) {
        LOGGER_ERROR(m->log, "handle gc sync request1");
        return -1;
    }
    ...
    if (!chat) {
        LOGGER_ERROR(m->log, "handle gc sync request2");
        return -1;
    }

    if (chat->connection_state <= CS_MANUALLY_DISCONNECTED || chat->shared_state.version == 0) {
        LOGGER_ERROR(m->log, "handle gc sync request3");
        return -1;
    }
    ...
    if (chat->shared_state.password_length > 0) {
        ...
        if (memcmp(chat->shared_state.password, password, chat->shared_state.password_length) != 0) {
            LOGGER_ERROR(m->log, "handle gc sync request4");
            return -1;
        }
    }
    ...
    if (send_peer_shared_state(chat, gconn) == -1) {
        LOGGER_ERROR(m->log, "handle gc sync request5");
        return -1;
    }

    if (send_peer_mod_list(chat, gconn) == -1) {
        LOGGER_ERROR(m->log, "handle gc sync request6");
        return -1;
    }
    ...
    uint8_t sender_relay_data[GC_MAX_PACKET_SIZE];
    net_pack_u32(sender_relay_data, chat->self_public_key_hash);
    int announce_length = gca_pack_announce(sender_relay_data + HASH_ID_BYTES, sizeof(sender_relay_data) - HASH_ID_BYTES,
                                            &new_peer_announce);
                                ...
                                uint32_t length)
{
    ...
    if (chat->connection_state != CS_CONNECTED) {
        LOGGER_ERROR(m->log, "handle_gc_tcp_relays() failed. State: %d", chat->connection_state);
        return -1;
    }
...
{
    uint8_t data[GC_MAX_PACKET_SIZE];
    uint32_t length = HASH_ID_BYTES;
...
FAILED_INVITE:
    ...
    gc_peer_delete(m, group_number, peer_number, nullptr, 0, false);
...
{
    if (length + GC_BROADCAST_ENC_HEADER_SIZE > GC_MAX_PACKET_SIZE) {
        return -1;
    }
...
static int handle_gc_peer_announcement(Messenger *m, int group_number, const uint8_t *data, uint32_t length)
{
    ...
    if (unpack_result == -1) {
        LOGGER_ERROR(m->log, "in handle_gc_peer_announcement unpack error");
        return -1;
    }
    ...
    if (!ip_port && !new_peer_announce.tcp_relays_count) {
        LOGGER_ERROR(m->log, "in handle_gc_peer_announcement invalid info");
        return -1;
    }
    ...
    if (peer_number == -2) {
        ...
        LOGGER_ERROR(m->log, "peer add failed");
        return -1;
    }
                                        ...
                                        const uint8_t *data, uint32_t length)
{
    ...
    if (unpack_gc_peer(&peer, data + MAX_GC_PASSWORD_SIZE, length - MAX_GC_PASSWORD_SIZE) == -1) {
        LOGGER_ERROR(m->log, "unpack_gc_peer failed in handle_gc_peer_info_request");
        return -1;
    }

    if (peer_update(m, group_number, &peer, peer_number) == -1) {
        LOGGER_ERROR(m->log, "peer_update() failed in handle_gc_peer_info_request");
        return -1;
    }
...
ON_ERROR:
    gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"BAD MLIST", 9, false);
        ...
        uint32_t peer_number, GC_Chat *chat)
{
    ...
    gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"BAD SCREDS", 10, false);
                               ...
                               uint32_t length)
{
    ...
    return gc_peer_delete(m, group_number, peer_number, data, length, false);
}
                          ...
                          uint32_t length)
{
    ...
    if (length == 0 || length > MAX_GC_NICK_SIZE || get_nick_peer_number(chat, nick, length) != -1) {
        return gc_peer_delete(m, group_number, peer_number, nullptr, 0, false);
    }
...
static int handle_gc_custom_packet(Messenger *m, int group_number, uint32_t peer_number, const uint8_t *data,
                                   uint32_t length)
{
    if (!data || length == 0 || length > GC_MAX_PACKET_SIZE) {
        return -1;
    }
                               ...
                               uint32_t length)
{
    ...
    if (gc_peer_delete(m, group_number, target_peer_number, nullptr, 0, false) == -1) {
        return -1;
    }
                                        ...
                                        const uint8_t *data, uint16_t length)
{
    ...
    switch (request_type) {
        ...
        default:
            LOGGER_ERROR(m->log, "Warning: received invalid request type in handle_gc_handshake_response");
            return -1;
    }
...
static int peer_reconnect(Messenger *m, const GC_Chat *chat, const uint8_t *peer_pk)
{
    ...
    gc_peer_delete(m, chat->group_number, peer_number, nullptr, 0, false);
                                       ...
                                       const uint8_t *data, uint32_t length)
{
    ...
    if (join_type == HJ_PUBLIC && !is_public_chat(chat)) {
        gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"Join priv", 9, false);
        return -1;
    }
 ...
 * Returns -1 on failure.
 */
static int handle_gc_handshake_packet(Messenger *m, GC_Chat *chat, const IP_Port *ipp, const uint8_t *packet,
                              ...
                              uint16_t length, uint64_t message_id, uint8_t packet_type)
{
    ...
    switch (packet_type) {
        ...
        default:
            LOGGER_ERROR(m->log, "Warning: handling invalid lossless group packet type %u", packet_type);
            return -1;
    }
}
...
static int handle_gc_lossless_message(Messenger *m, GC_Chat *chat, const uint8_t *packet, uint16_t length,
                                      bool direct_conn)
{
    if (length < GC_MIN_LOSSLESS_PACKET_SIZE || length > GC_MAX_PACKET_SIZE) {
        return -1;
    }
...
static int handle_gc_lossy_message(Messenger *m, GC_Chat *chat, const uint8_t *packet, uint16_t length,
                                   bool direct_conn)
{
    if (length < GC_MIN_LOSSY_PACKET_SIZE || length > GC_MAX_PACKET_SIZE) {
        return -1;
    }
...
static int peer_update(Messenger *m, int group_number, GC_GroupPeer *peer, uint32_t peer_number)
{
    ...
    if ((nick_num != -1 && nick_num != peer_number)) {   /* duplicate nick */
        ...
        gc_peer_delete(m, group_number, peer_number, (const uint8_t *)"dup nick", 8, false);
        return -1;
    }
...
static void do_peer_connections(Messenger *m, int group_number)
{
    ...
    for (i = 1; i < chat->numpeers; ++i) {
        ...
        if (peer_timed_out(m->mono_time, chat, &chat->gcc[i])) {
            gc_peer_delete(m, group_number, i, (const uint8_t *)"Timed out", 9, false);
        } else {
...
static bool gc_rejoin_connected_group(GC_Session *c, GC_Chat *chat)
{
    ...
    for (i = 1; chat->numpeers > 1;) {
        gc_peer_delete(c->messenger, chat->group_number, i, nullptr, 0, true);
    }
                                     ...
                                     uint32_t length)
{
    ...
    uint8_t send_data[GC_MAX_PACKET_SIZE];
